...
def main():
    ...
    model  = Informer(                                                           # Initialize Informer model parameters
    ...
    #---- Optim / Scheduler / AMP ----
    opt    = torch.optim.AdamW(model.parameters(),                              # Build AdamW Optimizer
                               lr=2.5e-4,
                               weight_decay=5e-3)
    scaler = GradScaler(enabled=True)                                           # Enable Auto-Hybrid Precision Training
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for bi, (X, y) in enumerate(tqdm(train_dl, desc=f"[Epoch {epoch:02d}]", leave=False)):
            ...
            with autocast():
                ...
                loss = F.mse_loss(pred, y) / ACCUM                                  # Calculate Mean Squared Error Loss
            # backward
            ...
            if (bi + 1) % ACCUM == 0:
                ...
                scaler.step(opt)
                scaler.update()                                                    # Update the optimizer and scaler
        ...
        with torch.no_grad(), autocast():
            for X, y in val_dl:
                ...
                n  += y.numel()
        val_rmse = math.sqrt(se / n)                                            # Calculate the average validation RMSE
    ...
    #---- Test ----
    model.load_state_dict(torch.load("best_informer.pt"))                      # Load optimal model parameters
    model.eval(); se = 0.0; n = 0
    with torch.no_grad(), autocast():
        for X, y in test_dl:
            X = X.to(device, non_blocking=True)
            ...
            se += F.mse_loss(pred, y, reduction='sum').item()
            n  += y.numel()
    print(f"\nBest val RMSE {best_rmse:.3f} | Test RMSE {math.sqrt(se/n):.3f}") #Print Test RMSE
