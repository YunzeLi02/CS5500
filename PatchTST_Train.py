...
P95_TH = (8.23  - mean_ws10) / std_ws10
P99_TH = (11.48 - mean_ws10) / std_ws10

def rmse(x, y): return (F.mse_loss(x, y).sqrt())
...
def eval_val(model, loader, device):
    ...
    rmse_series = math.sqrt(se_series / max(n_series, 1))
    rmse_grid   = math.sqrt(se_grid   / max(n_grid,   1))
...
def main():
    ...
    # ---- Data ----
    # 1) Construct LoaderCfg
    train_cfg = LoaderCfg(device="cpu", batch=8, num_workers=4, use_fp16=False)

    # 2) Then pass cfg as the sole argument to build_loaders.
   ...
   # ---- Optim / Scheduler ----
    ...
    for ep in range(1, epochs + 1):
        ...
        for x, y in pbar:
            ...
            if pred.dim() == 4:                                             # [B,T,H,W]
                ...
                if y_grid is not None:
                    loss = F.mse_loss(pred_grid, y_grid)                    # grid format
                else:
                    loss = F.mse_loss(pred_series, y_series)                # series format
        ...
        if val_rmse_series_z < best_val:
            ...
            torch.save(model.state_dict(), "patchtst_ws10_best.pt")
        else:
            patience += 1
            if patience >= max_pat:
                break
    ...
    # ---- Test ----
    model.load_state_dict(torch.load("patchtst_ws10_best.pt", map_location=device))
    model.eval()
    ...
    test_rmse_series_z = math.sqrt(se_series / n_series)
    test_rmse_grid_z = math.sqrt(se_grid / n_grid) if n_grid > 0 else float('nan')
