...
class ProbSparseSelfAttn(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int = 4, dropout: float = 0.1):
        ...
        self.q_proj = nn.Linear(d_model, d_model)                  # Query Projection Layer
        self.k_proj = nn.Linear(d_model, d_model)                  # Key Projection Layer
        self.v_proj = nn.Linear(d_model, d_model)                  # Value Projection Layer
        self.out    = nn.Linear(d_model, d_model)                  # Output layer
        self.drop   = nn.Dropout(dropout)                          # Dropout layer

    def forward(self, x: torch.Tensor):                       # (B,L,D)
        B, L, _ = x.shape
        k_top = int(math.ceil(math.sqrt(L)))                  # Calculate the top-k values
        k_top = min(k_top, L)                                 # Ensure top-k does not exceed the sequence length
        Q = self.q_proj(x).view(B, L, self.h, self.d).transpose(1, 2)  # (B,h,L,d) for query
        K = self.k_proj(x).view(B, L, self.h, self.d).transpose(1, 2)  # (B,h,L,d) for key
        V = self.v_proj(x).view(B, L, self.h, self.d).transpose(1, 2)  # (B,h,L,d) for value
...
class ProbSparseCrossAttn(nn.Module):
    ...
    def forward(self, Q_in: torch.Tensor, KV_in: torch.Tensor):
        B, Lq, _ = Q_in.shape
        _, Lkv, _ = KV_in.shape
        k_top = int(math.ceil(math.sqrt(Lkv)))                                 # Calculate the top-k values
        k_top = min(k_top, Lq)                                                 # Ensure top-k does not exceed the sequence length

        Q = self.q_proj(Q_in).view(B, Lq,  self.h, self.d).transpose(1, 2)    # (B,h,L,d) for query
        K = self.k_proj(KV_in).view(B, Lkv, self.h, self.d).transpose(1, 2)   # (B,h,L,d) for key
        V = self.v_proj(KV_in).view(B, Lkv, self.h, self.d).transpose(1, 2)   # (B,h,L,d) for value
...
class Informer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_embed = nn.Sequential(nn.Linear(in_dim, d_model), nn.Dropout(dropout)) # Encoder Embedding Layer
        self.dec_embed = nn.Sequential(nn.Linear(in_dim, d_model), nn.Dropout(dropout)) # Decoder Embedding Layer
