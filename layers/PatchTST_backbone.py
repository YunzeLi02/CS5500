...
class FusionHead(nn.Module):
    def __init__(self, n_vars, nf_per_var, target_window, head_dropout=0.0):
        super().__init__()
        in_features = n_vars * nf_per_var   # nf_per_var = d_model * patch_num
    ...
    def forward(self, x):                   # x: [B, nvars, d_model, patch_num]
        B, N, D, P = x.shape
        x = x.reshape(B, N * D * P)         # → [B, N*D*P]
        return self.proj(x)


class SpatialLowRankHead(nn.Module):


    def __init__(self, n_vars: int, nf_per_var: int, target_window: int,
                 H: int, W: int, rank: int = 16, dropout: float = 0.0):
        super().__init__()
        self.rank = rank
        in_dim = n_vars * nf_per_var           # = nvars * (d_model * patch_num)
        self.coef_mlp = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(in_dim, target_window * rank)  # → [B, T*R]
        )
        self.basis = nn.Parameter(torch.empty(rank, H, W))
        nn.init.xavier_uniform_(self.basis)
        self.T = target_window

    def forward(self, z):                     # z: [B, nvars, d_model, P]
        B, N, D, P = z.shape
        feat = z.reshape(B, N * D * P)        # [B, in_dim]
        coef = self.coef_mlp(feat).view(B, self.T, self.rank)  # [B,T,R]
        # [B,T,R] × [R,H,W] → [B,T,H,W]
        y = torch.einsum('btr,rhw->bthw', coef, self.basis)
        return y



class PatchTST_backbone(nn.Module):          #[B, T, C, H, W] ->[B, C, L]

    def _prep_input(self, z: torch.Tensor) -> torch.Tensor:
        while z.dim() > 3 and z.shape[-1] == 1:
            z = z.squeeze(-1)
        while z.dim() > 3:
            z = z.mean(dim=-1)

        if z.shape[1] == self.n_vars:
            return z.contiguous()
        elif z.shape[2] == self.n_vars:
            return z.permute(0, 2, 1).contiguous()
        else:
            raise RuntimeError(
                f"(Expected {self.n_vars}),Real{tuple(z.shape)}."
                "Please DataLoader Output."
            )
    ...
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        self.head = FusionHead(
            n_vars=self.n_vars,
            nf_per_var=self.head_nf,
            target_window=target_window,
            head_dropout=head_dropout
        )
        # Single-channel output label
        ...
        if self.out_mode == "series":
            #  Original: Cross-channel fusion → Single-variable sequence[B,T]
            self.head = FusionHead(
                n_vars=self.n_vars, nf_per_var=self.head_nf,
                target_window=target_window, head_dropout=head_dropout
            )
        elif self.out_mode == "grid":
            # New：Spatial low-rank encoder → grid output [B,T,H,W]
            self.spatial_head = SpatialLowRankHead(
                n_vars=self.n_vars, nf_per_var=self.head_nf,
                target_window=target_window,
                H=self.grid_H, W=self.grid_W,
                rank=self.spatial_rank, dropout=head_dropout
            )
        else:
            raise ValueError(f"out_mode must be 'series' or 'grid', got {self.out_mode}")

    def forward(self, z):  # z: [bs x nvars x seq_len]

        z = self._prep_input(z)
        # norm
        if self.revin:
        ...
        z = z.unfold(dimension=-1, size=self.patch_len, step=self.stride)  # [B, C, P, Lp]
        z = z.permute(0, 1, 3, 2)  # [B, C, Lp, P]

        # encoder+head
        z = self.backbone(z)  # [B, C, d_model, P]
        if self.out_mode == "series":
            y = self.head(z)
            return y
        else:
            # Spatial Prediction [B,T,H,W]
            y = self.spatial_head(z)
            return y
