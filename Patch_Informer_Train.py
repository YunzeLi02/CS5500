...
def main():
    ...
    opt    = torch.optim.AdamW(params, lr=1e-4, weight_decay=1e-3)
    epochs, clip = 20, 0.8
    ...
    for ep in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for X, y in val_dl:  # Loop val_dl
                ...
                X_aug = torch.cat([X, cond(X)], dim=2)
                pred = model(X_aug)
                pred = to_BTHW(pred)

                ps = pred.mean(dim=(-1, -2))  # [B,T]
                se_s += F.mse_loss(ps, ys, reduction='sum').item();
                n_s += ys.numel()
                se_g += F.mse_loss(pred, yg, reduction='sum').item();
                n_g += yg.numel()

        val_rmse_series = math.sqrt(se_s / max(n_s, 1))
        val_rmse_grid = math.sqrt(se_g / max(n_g, 1))
    ...
    # ---- Test（series & grid）----
    ckpt = torch.load("best_patchtst_informer.pt", map_location=device)
